  ...
  <div class="content-wrapper row">
    ...
    <section class="invoice">
      ...
      <table style='width:1200px;  border-spacing: 10px 0; '>
        <tr>
          <td >Domain : <span>{{$adjustout->branch->business_name.", ".$adjustout->branch->branch_name}}</span></td>
          <td style='width:50%'><span>Status : </span><span >{{$adjustout->approval->status}}</span></td>
        </tr>
        <tr>
          <td >Branch : <span>{{$adjustout->branch->branch_name}}</span></td>
          <td style='width:50%'><span>AdjustOut ID : </span><span >{{$adjustout->stock_adj_out_id}}</span></td>
        </tr>
        <tr>
          <td><span >Encode Date : </span><span>{{$adjustout->encode_date}}</span> </td>
          <td><span >PostDate : </span><span >{{$adjustout->post_date}}</span></td>
        </tr>

        <tr>
          <td><span >User: </span><span>{{$adjustout->user->username}}</span></td>
        </tr>
      </table>
      <!-- /.row -->

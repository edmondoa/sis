  ...
  <div class="content-wrapper row">
    ...
    <section class="invoice">
      ...
      <table style='width:1200px;  border-spacing: 10px 0; '>
        <tr>
          <td >Domain : <span>{{$adjustin->branch->business_name.", ".$adjustin->branch->branch_name}}</span></td>
          <td style='width:50%'><span>Status : </span><span >{{$adjustin->approval->status}}</span></td>
        </tr>
        <tr>
          <td >Branch : <span>{{$adjustin->branch->branch_name}}</span></td>
          <td style='width:50%'><span>AdjustIn ID : </span><span >{{$adjustin->stock_adj_in_id}}</span></td>
        </tr>
        <tr>
          <td><span >Encode Date : </span><span>{{$adjustin->encode_date}}</span> </td>
          <td><span >PostDate : </span><span >{{$adjustin->post_date}}</span></td>
        </tr>

        <tr>
          <td><span >User: </span><span>{{$adjustin->user->username}}</span></td>
        </tr>
      </table>
      <!-- /.row -->
